...
class Data:
    ...
    def create_circuit_network(self, name, intensity=0, loss=0.0, codec=''):
        """
        ...
        """
        self.networks.append(nw.Circuit(name, self.index_networks, intensity, loss, codec))
        self.index_networks += 1

    def create_package_network(self, name, intensity_voice=0, intensity_video=0, intensity_be=0, codec='',
        ...
        self.networks.append(nw.Package(name, self.index_networks, intensity_voice, intensity_video, intensity_be,
                                        codec, video_package_length, be_package_length))
        self.index_networks += 1

    def edit_network(self, name, index, intensity_voice=0, intensity_video=0, intensity_be=0, loss=0, codec='',
                     video_package_length=0, be_package_length=0):
        """
        ...
        if type(self.networks[index]) is nw.Circuit:
            self.networks[index].edit_network(name, intensity_voice, loss, codec)
        elif type(self.networks[index]) is nw.Package:
            self.networks[index].edit_network(name, intensity_voice, intensity_video, intensity_be, codec,
    ...
    def package_length(self):
        ...
        for network in self.networks:
            if type(network) is core.networks.Package:
                ...
                be_package_lengths.append(network.package_length_h264)
                be_package_probabilities.append(network.no_video_package/no_video_all)
    ...
    def test_no_2(self):
        self.create_circuit_network('adam', 240, 0.002, 'g.711')
        self.create_circuit_network('aaaa', 350, 0.002, 'g.711')
        self.create_circuit_network('b', 180, 0.002, 'g.729')
        self.create_package_network('ccc', 120, 100, 100, 'g.729', 100, 100)
        self.create_package_network('ccc', 120, 200, 200, 'g.729', 100, 100)

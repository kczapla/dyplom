...
class Matrix(Frame):
    def __init__(self, distribution, tittle='Matrix', parent=None, **extras):
    ...
    def make_matrix(self):
        ...
        for x in range(self.distribution.index_networks):
            entry = []
            row = Frame(self)
            row.pack(side=TOP)
            for y in range(self.distribution.index_networks):
...
if __name__ == "__main__":
        ...
        m = Matrix(d, 'Voice interest matrix', root)
        root.mainloop()